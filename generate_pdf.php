...
$sql = "SELECT emp_date.id as user_id,emp_date.first_name as first_name,emp_date.last_name as last_name,email_id,question_set_name,logo_file,question_description,answer,answer_in_detail,file_data,emp_location,manager,company_name,answer.visit_no as visit_no,start_time,end_time,answer.id as answer_id FROM tbl_answer_set_master answer LEFT JOIN tbl_question_set_master question on question.id = answer.question_id RIGHT JOIN tbl_employee_master emp_date on answer.users_id = emp_date.id WHERE answer.id = $primary_id and answer.users_id = $jms_uid";
...
$jms_question_set_name = $data['question_set_name'];
$jms_description = json_decode($data['question_description'], true);
$jms_emp_location = $data['emp_location'];
$jms_manager = $data['manager'];
$jms_logo_file = $data['logo_file'];
...
$path = 'img/' . $jms_logo_file; //'pdf_header_img.png';
